...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout {
	...
	public final void setRefreshing(boolean doScroll) {
		if (!isRefreshing()) {
			...
			new GetDataTask(onRefreshListener, false).execute();
		}
	}
		...
		case MotionEvent.ACTION_UP: {
			if (isBeingDragged) {
				...
				if (state == RELEASE_TO_REFRESH && null != onRefreshListener) {
					boolean isUp = setRefreshingInternal(true);
					new GetDataTask(onRefreshListener, isUp).execute();
				} else {
					smoothScrollTo(0);
				}
				return true;
	...
	private class GetDataTask extends AsyncTask<Void, Void, Boolean> {
		...
		public GetDataTask(OnRefreshListener onRefreshListener, boolean isUp) {
		...
		protected Boolean doInBackground(Void... params) {
			...
			if (isUp) {
				...
				message = onRefreshListener.onLoad();
			} else {
				message = onRefreshListener.onRefresh();
			}
			try {
				Thread.sleep(1000);
		...
		protected void onPostExecute(Boolean result) {
			if (isUp) {
				onRefreshListener.onLoadView(result);
			} else {
				onRefreshListener.onRefreshView(result);
			}
			onRefreshComplete();
			isRefresh = true;
	...
	protected boolean setRefreshingInternal(boolean doScroll) {
		...
		if (doScroll) {
			smoothScrollTo(currentMode == MODE_PULL_DOWN_TO_REFRESH
					|| currentMode == 0 ? -headerHeight : headerHeight);
		}
		return isUp;
	}
	...
	private void init(Context context, AttributeSet attrs) {
		...
		if (mode == MODE_PULL_UP_TO_REFRESH || mode == MODE_BOTH) {
			footerLayout = new LoadingLayout(context, MODE_PULL_UP_TO_REFRESH,
					footerUp, footerDown, refreshingLabel);
